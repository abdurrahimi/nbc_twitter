    ...
    public function actionIndex(){
        ...
        foreach($call as $value){
            ...
            if (!empty($value['entities']['urls'])) {
                    ...
                    if (!empty($value['entities']['user_mentions'])) {
                        $mention = "yes";
                    }else{
                        $mention = "no";
                    }
                    ...
                    /*======== START PROCCESS DATA HOAX APA BUKAN ========*/
                    if ($value['user']['verified'] <> FALSE) { // If user verified
                        $verified = "Yes";
                    }else{
                        $verified = "No";
                    }
                    $token = date("Ymdhis");
                    $dateCreated = date("Y-m-d",strtotime($value['user']['created_at']));
                    $dateCreated = new \Datetime($dateCreated);
                    $dateNow     = new \Datetime(date("Y-m-d"));
                    $difference  = $dateNow->diff( $dateCreated );
                    $difference  = (string)$difference->y;

                    $class_hoax = 0;
                    $class_hoax_info = "Pro";
                    if ($value['user']['followers_count'] > 250) { //If Followers > 250
                        if ($value['user']['friends_count'] < ($value['user']['followers_count'] * 2)) { //if following < 200% followers

                            $str = $value['user']['name'];
                            //echo var_dump(preg_replace('/[^0-9]/', '', $str));die;
                            $int = preg_replace('/[^0-9]/', '', $str);
                            if ($int == "") { // If name has no more than 3 numbers
                                $str2 = $value['user']['screen_name'];
                                $int2 = preg_replace('/[^0-9]/', '', $str2);
                                if ($int2 == "") { // If username has no more than 3 numbers
                                    //$yearCreated = date("Y",strtotime($value['user']['created_at']));
                                    if ($difference >= 1) { // If year min 1 year
                                        if ($value['user']['verified'] <> FALSE) { // If user verified
                                            $class_hoax = 1;
                                            $class_hoax_info = "Kontra";
                                        }
                                    }
                                }
                            }
                    ...
                    if($check <> null){
                        ...
                        //$modelcrawl->screen_name        = "@".$value['user']['screen_name'];
                       $modelcrawl->tweet_id           = $value['id_str'];
                        $modelcrawl->text               = $value['text'];
                        //$modelcrawl->followers_count    = $value['user']['followers_count'];
                        //$modelcrawl->following_count    = $value['user']['friends_count'];
                        //$modelcrawl->location           = $value['user']['location'];
                        $modelcrawl->created_post       = $value['created_at'];
                        //$modelcrawl->created_account    = $value['user']['created_at'];
                        //$modelcrawl->token_search       = $token;
                        //$modelcrawl->result_crawl_json  = json_encode($value);
                        //$modelcrawl->verified           = $verified;
                        $modelcrawl->different_year     = $difference;
                        $modelcrawl->keyword            = $keyword;
                        $modelcrawl->class_hoax_info    = $class_hoax_info;
                        
                        $modelcrawl->save(false);
                    }*/
        }

            return $this->render('../site/twitter/index', ['dataProvider' => $call,'modelcrawl' => $modelcrawl,'model' => $model]);        
        }
